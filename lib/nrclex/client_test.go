...
func TestNRCLexClient_Integration_Get(t *testing.T) {
	...
	resp, err := nrcClient.AnalyzeText("I'm in love, this is an amazing day! I'm so happy!")

	require.NoError(t, err)
	require.NotNil(t, resp)
	require.Greater(t, resp.EmotionScore.Joy, 0.0)
	require.Greater(t, resp.VaderEmotionScore.Compound, 0.0)
}
...
func TestNRCLexClient_AnalyzeText(t *testing.T) {
	...
	assert.Equal(t, 1.0, response.EmotionScore.Anticipation)
	assert.Equal(t, 2.0, response.EmotionScore.Joy)
}
