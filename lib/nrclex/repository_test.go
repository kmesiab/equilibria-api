...
func TestRepository_FindRangeByUserID(t *testing.T) {
	db, mock, err := setupMockDB()
	require.NoError(t, err)
	repo := NewRepository(db)
	...
	endDate := time.Date(2021, 1, 31, 23, 59, 59, 0, time.UTC)

	rows := sqlmock.NewRows([]string{"id", "user_id", "message_id", "anger", "joy", "created_at"}).
		AddRow(1, userID, 2, 0.5, 0.5, time.Now()).
		AddRow(2, userID, 3, 0.6, 0.4, time.Now()) // Example row data

	mock.ExpectQuery("SELECT \\* FROM `nrclex` WHERE").
		WithArgs(userID, startDate, endDate, limit, offset).
		WillReturnRows(rows)

	result, err := repo.FindRangeByUserID(userID, limit, offset, startDate, endDate)

	require.NoError(t, err)
	assert.NotEmpty(t, result)
	assert.Len(t, result, 2) // Asserting that we found 2 records for demonstration purposes
